...
import { TEST_EMBEDDING_DIMENSIONS } from "./blackiron-seed-contract";
...
export type LastplayerSeedContract = {
  ids: {
    userId: string;
    projectId: string;
  };
...
const SEED_SOURCE_PATH = join(process.cwd(), "tests/fixtures/search/lastplayer-seed-source.json");
const FIXED_NOW = "2026-05-22T00:00:00.000Z";
const PROJECT_KEY = "lastplayer";
...
function deterministicTestEmbedding(seed: string): number[] {
  const values: number[] = [];
  let counter = 0;
...
function readSeedSource(): LastplayerSeedSource {
  return JSON.parse(readFileSync(SEED_SOURCE_PATH, "utf8")) as LastplayerSeedSource;
}
...
export function buildLastplayerSeedContract(): LastplayerSeedContract {
  ...
  const project: ProjectInsert = {
    ...
    title: source.title,
    genre: source.genre,
    description: `Search/RAG DB-backed evaluation seed derived from ${source.source}.`,
    metadata: {
      fixture: source.source,
      source: "tests/fixtures/search/lastplayer-seed-source.json",
    },
  ...
  const chapters = source.chapters.map((sourceChapter): ChapterInsert => {
  ...
  const entities: EntityInsert[] = source.entities.map((entity) => ({
  ...
  const entityLinks = source.relations
  ...
  return {
    ...
    expectedQueries: [
      {
        ...
        query: "주인공",
        expectedEntityIds: [entityIdsByName["주인공"]],
        expectedChapterIds: [],
        acceptableTopK: 8,
      },
      {
        ...
        query: "제국 노예제도",
        expectedEntityIds: [entityIdsByName["제국"]],
        expectedChapterIds: [chapterIdsByNumber[2]],
        acceptableTopK: 8,
      },
      {
        id: "lastplayer-monster-forest",
        query: "숲 괴수 무리",
        expectedEntityIds: [entityIdsByName["괴수"]],
        expectedChapterIds: [chapterIdsByNumber[3]],
        acceptableTopK: 8,
      },
    ],
    expectedVectorQueries: [
      ...
      },
      {
        ...
        query: "어두운 숲에서 괴수 무리가 습격하는 장면",
        queryEmbedding: deterministicTestEmbedding("chunk:3:0"),
        expectedChunkIds: [chunkIdsByKey["3:0"]],
        acceptableTopK: 3,
      },
    ],
    expectedGraphQueries: [
      {
        ...
        startEntityId: entityIdsByName["주인공"],
        maxDepth: 2,
        expectedEntityIds: [entityIdsByName["제국"], entityIdsByName["수인"], entityIdsByName["괴수"]],
      },
    ],
  };
}
